...
public class LocalMoveOnMouse : MonoBehaviour, IPointerEnterHandler, IPointerMoveHandler, IPointerExitHandler
{
    ...
    private void Start()
    {
        position = moveObj.localPosition;
    }

    public void OnPointerEnter(PointerEventData eventData)
    {
        onArea = true;
        moveObj.DOLocalMove(position + movementVector, enterEaseTime).SetEase(enterEase);
    }
    ...
    {
        onArea = false;
        moveObj.DOLocalMove(position, exitEaseTime).SetEase(exitEase);
    }
}
